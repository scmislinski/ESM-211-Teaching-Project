...
## Compare the Model Projections and Response Curves of the Milkweed and Monarch Data Sets
...
The model predicts the presence of milkweed in several locations where monarchs are not predicted to occur, including inland and to the north of Santa Barbara and the Channel Islands. It is probable that milkweed can thrive in a wider range of climatic conditions (precipitation and temperature) in which it can exist, as the presence of milkweed is not the only determinant of monarch presence.
...
While milkweed is an important food source for monarch caterpillars, it is not the only food source for monarch butterflies. The butterflies have a variety of food sources, included nectar from flowers and eucalyptus. Monarchs are migratory, and the occurrence data that the model uses may not all come from locations where the species is in the larval stage.
